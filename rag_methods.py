...
# Func to stream the response of the llm
                ...
                if len(st.session_state.rag_sources) < DB_DOCS_LIMIT:
                    ...
                    try:
                        if doc_file.type == "application/pdf":
                            ...
                            st.warning(f"Document {doc_file.type} not supported.")
                            continue
...
def load_url_to_db():
    if "rag_url" in st.session_state and st.session_state.rag_url:
        ...
        docs = []
        if url not in st.session_state and st.session_state.rag_url:
            if len(st.session_state.rag_sources) < 10:
                ...
                if docs:
                    _split_and_load_docs(docs)
                    st.toast(f"Document from URL *{url}* loaded succesfully.", icon="✅")
            else:
...
def initialize_vector_db(docs):
    ...
    # keep only the collections for the last 20 users
